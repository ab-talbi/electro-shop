...
<?php
    ...
    function getProduits(){
        ...
        if(!isset($_GET['categorie']) && !isset($_GET['marque']) && !isset($_get['search_btn'])){
            $select_produit = $con->query('SELECT * FROM produits WHERE status_produit LIKE "disponible" order by rand() limit 0,12');
    ...
    function getTousProduits(){
        ...
        if(!isset($_GET['categorie']) && !isset($_GET['marque'])){
            $select_produit = $con->query('SELECT * FROM produits WHERE status_produit LIKE "disponible" order by rand()');
            while($ligne = $select_produit->fetch(PDO::FETCH_OBJ)){
                echo "<div class='col-md-3 mb-2 mt-2'>
                    <div class='card'>
                    <img src='/Electro-Shop/admin/produits_images/$ligne->produit_image1' class='card-img-top' alt='$ligne->nom_produit'>
    ...
    function getProduitsByCategorie(){
        ...
        if(isset($_GET['categorie']) && !isset($_GET['marque'])){
            ...
            $select_produit = $con->query("SELECT * FROM produits WHERE status_produit LIKE 'disponible' AND id_categorie = $categorie_id");
    ...
    function getProduitsByMarque(){
        ...
        if(isset($_GET['marque']) && !isset($_GET['categorie'])){
            ...
            $select_produit = $con->query("SELECT * FROM produits WHERE status_produit LIKE 'disponible' AND id_marque = $marque_id");
            $rows = $select_produit->rowCount();
            if($rows == 0){
    ...
    function getProduitsByMarqueEtCategorie(){
        ...
        if(isset($_GET['marque']) && isset($_GET['categorie'])){
            $marque_id = htmlspecialchars($_GET['marque']);
            $categorie_id = htmlspecialchars($_GET['categorie']);
            $select_produit = $con->query("SELECT * FROM produits WHERE id_marque = $marque_id AND id_categorie = $categorie_id AND status_produit LIKE 'disponible'");
            $rows = $select_produit->rowCount();
            if($rows == 0){
    ...
    function searchProducts(){
        ...
        if(isset($_GET['search_btn'])){
            ...
            $select_produit = $con->query("SELECT * FROM produits WHERE mots_cles LIKE '%$search_data%' AND status_produit LIKE 'disponible'");
            $rows = $select_produit->rowCount();
            if($rows == 0){
